...
/*   Updated: 2019/12/25 18:52:37 by fbouibao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	norme_prec_x(vr_list *vrbs, t_list *tmp)
{
	if ((int)ft_strlen(vrbs->val) > vrbs->prec)
	{
...
int			str_spf_x(va_list ap, t_list *tmp)
{
	...
	if (vrbs->flgs[0] == '\0')
	{
		if (!(tmp->str = ft_itoa_int16((unsigned int)va_arg(ap, unsigned int), 0, 0)))
			return (0);
		free_vrbs(vrbs);
		return (1);
	}
